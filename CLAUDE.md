...
- Progress: Phase 1 is **code-complete** — §1.1 scaffold, §1.2 detection engine, §1.3 NO_SIGNAL calibration, §1.4 popup/badge/options UI (EN source + IT locale), §1.5 automated hardening, plus one owner field-test round (129 unit tests incl. real spike-capture fixtures, the invariant-2 payload test, and a reconstructed field-bug fixture); pre-release security audit passed. Field-test outcomes: rewatch NO_SIGNAL no longer clobbers the last informative history entry; fast back/forward handled (confirmation-read ladder, bfcache pageshow); popup distinguishes placement-only "ad decision" from observed playback; user-initiated "Export JSON" (versioned `adsauditor-local-export` payload); **badge sightings are weak B evidence** — YouTube ships empty `ytp-ad-*` scaffolding on non-monetized videos, so badge-only yields NO_SIGNAL `anomalous-ad-ui-only`, never ADS_SERVED and never NO_ADS (SPEC §3.2 "strong vs. weak B"). Observer validity comes from `extension/utils/calibration.ts` (adblock bait probe with redirect-to-stub detection, Premium heuristic — selector still unverified —, passive control-video calibration, 72 h positive-evidence window, 24 h rewatch index). Next: the §1.5 **manual** checklist on Chrome + Firefox (owner-run gate; item 6 was reworded for the SPEC §3.4 probes and needs owner sign-off) → tag v0.1.0.
