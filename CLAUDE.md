...
Open source, **local-only** observer of YouTube ad delivery: a browser extension (Chrome/Edge/Firefox, Manifest V3) that detects whether a video is actually serving ads, entirely on the user's device. **Direction update (2026-07-11, owner decision): the opt-in crowdsourced database (Supabase) and public dashboard (Next.js) are descoped to "possible future feature"** — data sharing is manual (popup "Export JSON" → contact the maintainer). SPEC §4-6 and ROADMAP Phases 2-4 remain in the repo as design history for that possible future.
...
- Progress: Phase 1 is **complete and published** — detection engine, NO_SIGNAL calibration, popup/badge/options UI (EN source + IT locale), 131 unit tests (real spike-capture fixtures, the invariant-2 payload test, field-bug regression fixtures), pre-release security audit passed, two owner field-test rounds absorbed. Key field-learned rule: `ad-badge-seen` sightings are **diagnostics-only** — YouTube ships empty `ytp-ad-*` scaffolding on essentially every video, so badges contribute nothing to verdicts; only ad-state class transitions are strong B evidence (SPEC §3.2 "strong vs. weak B"). Observer validity comes from `extension/utils/calibration.ts` (adblock bait probe with redirect-to-stub detection, Premium heuristic — selector still unverified —, passive control-video calibration, 72 h positive-evidence window, 24 h rewatch index); the badge shows the last valid observation on rewatches. Public repo: github.com/Vadale/adsauditor. §1.5 checklist executed in-browser on Chromium via the e2e harness (2026-07-11): green video ✓, ad-free/limited pattern ✓ (Elisa videos → NO_ADS), unavailable → UNAVAILABLE ✓, uBlock Origin Lite → NO_SIGNAL 'adblock-suspected' with the probe reporting 'blocked' ✓, SPA×5 with session/badge always matching the page ✓, network census clean (only YouTube/Google-ecosystem origins; probes as documented) ✓, control video still green ✓. **Firefox not verified in-browser** (no Firefox installed on the dev machine; Playwright can't load extensions in its Firefox). Field note for future threshold work: dataset "high-confidence yellow" pBpUC4Z-ZAY currently serves 17 placements (~0.22 midrolls/min, between green ~0.45 and hypothesized limited <0.1) — public-complaint labels age. v0.1.0 released 2026-07-11 (owner-confirmed gate: Chromium-verified end-to-end; the Firefox build ships explicitly marked as not fully verified).
...
- The owner runs no further manual collection; any future threshold calibration relies on manually shared export files (the crowdsourced pipeline is descoped).
- Field-test round (2026-07-11) shipped: rewatch history preservation, fast-navigation robustness, badge-scaffolding fix (badges are diagnostics-only), badge shows last valid observation on rewatches, orphaned-content-script hardening, popup "Export JSON" (versioned `adsauditor-local-export` payload). Manifest version 0.1.0 is the build-identity tell.
