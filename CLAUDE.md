...
- Progress: Phase 1 is **code-complete** — §1.1 scaffold, §1.2 detection engine, §1.3 NO_SIGNAL calibration, §1.4 popup/badge/options UI (EN source + IT locale), §1.5 automated hardening (103 unit tests incl. real spike-capture fixtures and the invariant-2 payload test); pre-release security audit passed (boundary caps added, docs drift fixed, `docs/PRIVACY.md` created). Observer validity comes from `extension/utils/calibration.ts`: adblock bait probe (with redirect-to-stub detection), Premium masthead heuristic (selector unverified, degrades to not-detected), passive control-video calibration, 72 h positive-evidence window, 24 h rewatch index; absences from invalid/uncalibrated observers surface as NO_SIGNAL with a specific cause, never NO_ADS. Next: the §1.5 **manual** checklist on Chrome + Firefox (owner-run gate; item 6 was reworded for the SPEC §3.4 probes and needs owner sign-off) → tag v0.1.0.
